...
type
  ...
  public
    ...
    function RenderArticleView(
      ...
      var Article: TOrmArticle;
      var Author: variant;
      var Comments: TObjectList): RawUtf8;
    procedure AuthorView(
      var ID: TID;
      var Author: TOrmAuthor;
      out Articles: variant);
...
function TBlogApplication.RenderArticleView(ID: TID; var WithComments: boolean;
  Direction: integer; var Scope: variant; var Article: TOrmArticle;
  var Author: variant; var Comments: TObjectList): RawUtf8;
var
  newID: Int64;
...
begin
  ...
  begin
    ...
    mustache := TSynMustache.Create('ArticleView'); 
    var d: RawUtf8 := AnyTextFileToRawUtf8('views\data.partial');
    var ContextData: variant := ObjectToVariant(Article, true);
    var p := TSynMustache.Parse(d).Render(Variant(contextData));
    result := RawUtf8ToVariant(p);    
  end
  else
    raise EMvcApplication.CreateGotoError(HTTP_NOTFOUND);
end;
